...
class JmeterWriter(JmeterTemplate):
        ...
        for request_data in select_data:
            ...
            if request_data['method'].lower() == 'get':
                all_request = all_request + self.get_sampler.format(
                    ...
                    # port_number = "${" + str(domain[request_data['port_number']]) + "}",  # setting domain,
    ...
    def __set_test_plan2(self, select_data):
        """
        组装test plan部分，设置domain和port, 后续进行${domain}和${port}参数化
        :param select_data:
        :return:
        """
        test_plan_domain_value = ''
        ...
        for data in select_data:
            ...
            if server_name not in domain:
                domain_value = "domian" + str(i)
                domain[server_name] = (domain_value, data['port_number'] or '')
                test_plan_domain_value += self.test_plan_value.format(domain_value=domain_value, domain=server_name,
                                                                      port=data['port_number'] or '')
                i += 1
        return self.test_plan.format(test_plan_value=test_plan_domain_value), domain
